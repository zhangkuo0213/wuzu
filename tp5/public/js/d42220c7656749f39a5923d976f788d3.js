...
$(function() {
    ...
    var $filter = $('#conditionSort');
    var $filterNav = $filter.find('.sortItem>.sortNav');
    var $filterItem = $filter.find('.popup');
    var filterParams = {
        brand: '',
        type: '',
        color: '',
    ...
    // 首次获取数据
    $content.find('ul').eq(0).addClass('container');
    getContent();

    // 阻止筛选<a>的默认事件
    $filterItem.find('.sortByCha a').addClass('touchToClick').on("click", function(event) {
        var toHash = $(this).attr('href');
        $(this).addClass('cur').siblings().removeClass('cur');
        $(window).scrollTop($(toHash).offset().top);
        event.preventDefault();
    });

    // 筛选区域收缩动画
    $filterNav.addClass('touchToClick').on("click", function() {
        var nowIndex = $filterNav.find('.selectShow.cur').parent().index();
        var thisIndex = $(this).index();
        if (nowIndex == thisIndex) {
            freeContent();
            $(this).find('.selectShow').removeClass('cur');
            $filterItem.eq(thisIndex).stop().slideUp(function() {
                $.dxlBackgroundHide();
            });
        } else {
            lockContent();
            $filterNav.find('.selectShow.cur').removeClass('cur');
            $(this).find('.selectShow').addClass('cur');
            $.dxlBackgroundShow();
            $filterItem.eq(thisIndex).stop().slideDown().siblings('.popup').hide();
        }
    });

    // 筛选：品牌
    var $filterBrand = $filterItem.eq(0);
    $filterBrand.find('.brandType').addClass('touchToClick').on("click", function() {
        /**/
        $filterOther.find('.filterType').eq(0).addClass('cur').siblings().removeClass('cur');
        filterParams.type = '';
        $filterOther.find('.filterColor').eq(0).addClass('cur').siblings().removeClass('cur');
        filterParams.color = '';
        /**/
        var brand = $(this).attr('data-brand') || '';
        $filterNav.eq(0).trigger('touchend');
        if (brand == $filterBrand.data('brand')) return; // 没变动，不做处理
        $filterBrand.find('.brandType').removeClass('cur'); // 删除已选标识，热门和列表
        $(this).addClass('cur');
        $filterBrand.data('brand', filterParams.brand = brand);
        filterParams.type = filterParams.color = filterParams.price = '';
        getContent(true);
    });
    $filterBrand.find('.nores').addClass('cur');

    // 筛选：排序
    var $filterSort = $filterItem.eq(1);
    $filterSort.find('.sortBy').addClass('touchToClick').on("click", function() {
        $filterNav.eq(1).trigger('touchend');
        $(this).addClass('cur').siblings().removeClass('cur');
        var sort = $(this).attr('data-sort') || '';
        filterParams.sortBy = sort.split('|')[0] || '';
        filterParams.sortOrder = sort.split('|')[1] || '';
        getContent(true);
    });

    // 筛选：类型，价格
    var $filterOther = $filterItem.eq(2);
    $filterOther.find('.filterType:eq(0)').addClass('cur').attr('data-type', '');
    $filterOther.find('.filterColor:eq(0)').addClass('cur').attr('data-color', '');
    $filterOther.find('.filterType, .filterColor').addClass('touchToClick').on("click", function() {
        /**/
        var typeNotColor = $(this).hasClass('filterType');
        $filterBrand.find('.brandType').removeClass('cur');
        $filterBrand.find('.nores').addClass('cur');
        filterParams.brand = '';
        $filterOther.find(typeNotColor ? '.filterColor' : '.filterType').eq(0).addClass('cur').siblings().removeClass('cur');
        filterParams[(typeNotColor ? "color" : "type")] = '';
        /**/
        $(this).addClass('cur').siblings().removeClass('cur');
        var type = $filterOther.find('.filterType.cur').attr('data-type');
        var color = $filterOther.find('.filterColor.cur').attr('data-color');
        $filterNav.eq(2).trigger('touchend');
        if (type != filterParams.type || color != filterParams.color) {
    ...
    function getContent(reset) {
        reset && $content.find('ul').empty();
        // 瀑布流获取数据
        $.dxlInclud(["fall"], function() {
			// 20160309 for zhulei
            var url = '/' + ($.cookie('city') || 'sh') + '/HunChe/json?act=getCars&' + $.param(filterParams);
            var template = function(data, dom) {
                var html = '<li><a href="' + data.url + '" class="resultListItem"><dl class="clearfix"><dt><img src="' + data.img + '-wapliebiao' + '"></dt><dd><div class="list-detail-word"><span>' + data.series + '</span><span class="price">￥' + data.price + '/半天</span></div></dd><div class="fire">' + data.hot + '</div></dl></a></li>';
                $(dom).append(html);
            };
            var setting = {
                url: url,
                ...
                initNum: 10, // 初始化获取数量
                newNum: 10, // 每次新获取数量
                imgUrl: 'img', // 预加载图片源地址名称，如需加载图片地址在data.imgSrc属性中，则选取"imgSrc"作为该属性的值
                watchHeight: 50, // 页面离底部多远拉取数据，单位px（可省略）
                badImgUrl: "" // 坏图处理，无图片或图片加载出错时展示
            };
            var haddle = {
                'onLoading': function() {
                    $content.find('.nomore, .noResult').hide();
                },
                'onLoadingOk': function() {},
                'onComplete': function() {
                    $content.find('.nomore').show();
                    $content.find('.noResult').hide();
                },
                'onNoData': function() {
                    $content.find('.nomore').hide();
                    $content.find('.noResult').show();
                }
            };
            var option = {
                "setting": setting,
    ...
    function lockContent() {
        var limitHeight = $(window).height() - $header.outerHeight(true) - $filterNav.parent().outerHeight(true);
        $content.css('max-height', limitHeight);
    }
