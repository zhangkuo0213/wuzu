...
function pagePop(frontPage,backPage){
    ...
    var book = $(".book"),
        orderMiyue = $("#orderMiyue"),
        view = $("#view"),
        getCode = $(".getcode"),
        bodyW = $(window).width(),
        bodyH = $(window).height(),
        mobile = $(".form-item .mobile"),
        checkCode = $(".form-item .checkcode");
    //初始化

    orderMiyue.find(".main").css({
        "height": bodyH - 123
    })
    ...
    book.touchClick(function() {
        pagePop(orderMiyue, view);
    })

    $(".form-group .sort li").touchClick(function() {
        $(this).toggleClass("selected")
    })

    $("#destination,#travelDate").one("click", function() {
        $(this).removeClass("placeholder");
    })

    $("#orderSubmit").touchClick(function() {
        var city = $.cookie('city') || 'sh';
        var order_from = city + '_wap_MiYueDetail_1stIndex_ 3steps_TopRight';
        var destination = $("#destination option:selected").text();
        var travelDate = $("#travelDate option:selected").text();
        var styleArr = [];
        $(".form-group .sort li.selected").each(function() {
            var _this = $(this);
            styleArr.push(_this.find("h4").text()).toString().replace(/,/g, "-");
        })
