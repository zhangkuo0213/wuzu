...
;(function($){
	$.extend({
		"wPrevBack":function(){
			/*$(document).touchClick("header .prev",function(e){  //返回 history 上一步
				e.preventDefault();
				e.stopPropagation();
				window.history.back();
			})	*/	
		},
		"wFootAppDown":function(){	  //底部app下载
			$("footer .footAppDown").touchClick(function(){
				window.location = dxlHttp.m + "app?utm_source=App&utm_content=Wap&utm_campaign=Bottom"; 
			});
		},
		"mSearch":function(cb){   //点击搜索 
			$.dxlInclud(["M-search"],function(){
				$("header .riIcon .search").touchClick(function(event){
					event.stopPropagation();
					$.cookie("weddingListUrl",window.location.href,{expires:7,domain:"daoxila." + s4Com,path:"/" });  //记录底板页返回 值
					$.dxlWapSearch();
					$.isFunction(cb) ? cb() : "";
				})
			})
