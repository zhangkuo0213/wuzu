...
;(function($){
	$.extend({
		...
		"mBotAppDownload":function(){  //底部 点击下载app
			$("footer .footAppDown , #appDownBot .download").touchClick(function(ev){
				ev.preventDefault();
		...
		"mSearch":function(cb){   //点击搜索 
			$.dxlInclud(["M-search"],function(){
				$("header .search").touchClick(function(event){
					event.stopPropagation();
					$.cookie("hotelListUrl",window.location.href,{expires:7,domain:"daoxila." + s4Com,path:"/" }); //记录底板页返回 值
					$.dxlWapSearch();
					$.isFunction(cb) ? cb() : "";
				})
			})
