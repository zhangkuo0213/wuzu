...
$(function(){
    var priceItem = $(".price-list .item"), //价格
        collect = $(".toolbar .icon-star"), //收藏
        book = $(".toolbar .btn"),          //预定
        collectAction = dxlHttp.my + "ucenterjsonp/?act=onFavorite&", //收藏接口
        viewable = $("#viewable"),  //主页面
        reservationPage = $('#reservationPage'),    //查询席位页面
        helpPage = $('#helpPage'),    //查询席位页面
        picShowPage = $("#picShow"),    //大图
        bodyW = $(window).width();
        bodyH = $(window).height();

        var productName = $(".product-info h3");//产品名称

    //初始化
    if($(".toolbar").size()){
        var fixedDomTop = $(".toolbar").offset().top; //元素距离文档流的位置
    }
    $(".page-container").css("width",bodyW);
    $(".flight").css("width",bodyW-15);
    $(".help-nav li").css("height",40+"px");
    $(".help-content").css({'height':bodyH-43,"width":$(".help-content li").width()});
    $(".help-content li .text").css("height",bodyH-43+"px");
    $(".help-content li:first-child .text").css({"height":bodyH-104});
    //窗口变化
    $(window).on("resize orientationchange", function() {
        bodyW2 = $(window).width();
        bodyH2 = $(window).height();
        if($(".toolbar").size()){
            fixedDomTop = $(".toolbar").offset().top || fixedDomTop;  //横竖屏切换时，重置fixed元素top值 
        }
        positionSet();
        $(".page-container").css("width",bodyW2);
        $(".flight").css("width",bodyW2-15);
        //须知
        $(".help-content").css({'height':bodyH2-43});
        $(".help-content li .text").css("height",bodyH2-43+"px");
        $(".help-content li:first-child .text").css({"height":bodyH2-104});
      })
    $(window).on({
    ...
    if(priceItem.length>1){
        $(".price-list").addClass("more");
        $(".price-list").touchClick(function(){
            $(this).toggleClass("show");
        })
        priceItem.touchClick(function(){
            $(this).prependTo(".price-list");
        });
    }
    ...
    function positionSet(){ 
        ...
        var fixedDom = $(".toolbar");
        var posH = fixedDom.height(); 
        if( st >= 425 ){
    ...
    //查看席位 下单
    book.touchClick(function(){
        $.dxlInclud(["M-miyueOrder"],function(){
            $.mMiyueOrder({
                "subname":$(".product-info h3").text(),
                "sku.name":$(".price-list .item:first-child .tag").text(),
                "sku.displayPrice":$(".price-list .item:first-child .price strong").text(),
                "order_from":"Wap_MiYueDetail_Booking_BottomRight",
                "mobile":$("#loginByCode .enterUl .mobile").val()
            });
        })
        $.dxlGaPageTracker("/VPTracker/WapMiYue/Order/SubMobile-common?orderType=Booking");  //虚拟url
    });
    //大图展示
    $(".product-img").touchClick(function(){
        pagePop(picShowPage,viewable);
        $('html,body').scrollTop(0);
        $('#picShow .pic-list ul').css({'margin-top':-$('.pic-list li').height()/2,'width': bodyW * $('#picShow .pic').length});
        $('#picShow .pic-list li').css('width', bodyW);
        $('#picShow .pic-list').css({'width':bodyW});
    })
    $.dxlInclud(["dxlTouchEvent"], function(){
        $('#picShow .pic-list').dxlTouchEvent({
            tag: 'ul',
            tagList: '.pic',
            distance: bodyW,
        });
    ...
    //特色图展示
    $(".define .mod .img-box").touchClick(function(){
        var data_id = $(this).attr("data_id");
        var picShowMod = $("#picShowMod"+data_id);
        pagePop(picShowMod,viewable);
        defineImg(data_id);
        $('.focus').css({'margin-top':-$('.focus .bd li').height()/2,'width':$('.focus .bd li').width()});
    })
    function defineImg(data_id){
        $.dxlInclud(["touchSlider"],function(){
            $.touchSlider({ 
                slideCell:"#focus"+data_id,
    ...
    // 航班线路
    $(".flight-line .tab-nav li:first-child").addClass("current");
    $.dxlInclud(["touchSlider"],function(){
        var _index = $(".flight-line .tab-nav li.current").index();
        $.touchSlider({
            slideCell:"#tabBox",
            titCell:"#tabDot"+_index+ " ul", //开启自动分页 autoPage:true ，此时设置 titCell 为导航元素包裹层
            mainCell:"#tabPanel"+_index+ " ul",
            effect:"left",
    ...
    $(".flight-line .tab-nav li").touchClick(function(){
        _index = $(".flight-line .tab-nav li").index(this);
        $(this).addClass("current").siblings().removeClass("current");
        $(".flight-line .tab-panel").eq(_index).css({"display":"block"}).siblings().css({"display":"none"})
        $.touchSlider({
            slideCell:"#tabBox",
            titCell:"#tabDot"+_index+ " ul", //开启自动分页 autoPage:true ，此时设置 titCell 为导航元素包裹层
            mainCell:"#tabPanel"+_index+ " ul",
    ...
    if($("#tabBox .tab-cont>div").length > 1){  //航班线路
        $("#tabBox .tab-nav").show();
    }else{
        $("#tabBox .tab-nav").hide();
    }
    // 预定须知
    $(".url-list li").touchClick(function(){
        pagePop(helpPage,viewable);
        var _index = $(this).index();
        notice(_index);
        if(_index==0){
            $.dxlGaPageTracker("/VPTracker/WapMiYue/Visa-Intro");
        }else if(_index==1){
            $.dxlGaPageTracker("/VPTracker/WapMiYue/Cost-Intro");
        }else if(_index==2){
            $.dxlGaPageTracker("/VPTracker/WapMiYue/Book-Intro");
        }
    })
    function notice(_index){
        $.dxlInclud(["touchSlider"],function(){
            $.touchSlider({ 
                slideCell:"#helpPage",
                titCell:".help-nav ul", //开启自动分页 autoPage:true ，此时设置 titCell 为导航元素包裹层
                mainCell:".help-content ul", 
                effect:"left", 
                titOnClassName:"current",
                defaultIndex:_index,
                autoPage:true,
                endFun:function(i,c){ // 切换当前标题
                    $(".help-content .btn").touchClick(function(){
                        $.dxlInclud(["M-miyueOrder"],function(){
                            $.mMiyueOrder({
                                "subname":$(".product-info h3").text(),
                                "sku.name":$(".price-list .item:first-child .tag").text(),
                                "sku.displayPrice":$(".price-list .item:first-child .price strong").text(),
                                "order_from":"Wap_MiYueDetail_Visa_MidCenter",
                                "mobile":$("#loginByCode .enterUl .mobile").val()
                            });
                        })
                        $.dxlGaPageTracker("/VPTracker/WapMiYue/Order/SubMobile-common?orderType=Visa");  //虚拟url
                    });
                }
            });
        });
    ...
    //分享新浪微博
    $(".icon-share").touchClick(function(){ 
        $.dxlGaPageTracker("/VPTracker/MiYue/Tours/Share");
        var pageUrl = location.href;
        var shareWord = "我在@到喜啦 上看中了期待已久的蜜月游线路"+ pageUrl +"，带爱去旅行，甜蜜我们的世界，你也来看看吧！";
        var logoUrl = dxlHttp.s4 + "public/img/logo/wap.png";
        $.dxlWeiboShare(shareWord , logoUrl);
    });

    function pagePop(frontPage,backPage){
        frontPage.css({'-webkit-transform':'translate3D(0,0,0)','transform':'translate3D(0,0,0)','top':'0'})
        $("body").css({'height':bodyH,'overflow-y':'hidden'});
        backPage.find(".prev").css('pointer-events','none');
        $(".toolbar .icon-star,.toolbar .icon-tel").css('pointer-events','none');
        $.dxlBackgroundShow();
        frontPage.find(".prev").touchClick(function(){
            frontPage.css({'-webkit-transform':'translate3D(100%,0,0)','transform':'translate3D(100%,0,0)','top':'0'});
            $("body").removeAttr("style");
            $.dxlBackgroundHide();
            setTimeout(function(){ //动画结束时重置class
                backPage.find(".prev").removeAttr("style");
                $(".toolbar .icon-star,.toolbar .icon-tel").removeAttr("style");
            }, 400)
        });
    };
})
