	...
	$.extend({
		...
		**/
		"loginAndComment" : function(){
			...
			function loginAction(){  //登录或注册后的操作
				window.location.href = dxlHttp.m + $(".toCommentNow").attr("dataUrl");
			}
		}
	})
})($)
...
$(function(){
	...
	//2017.03.20   读取cookie  (dxl ==2? ==2删除 header 和 footer)  && dxlm  是否存在， 存在修改页面手机号------------
	var dxlUrlValue2 = $.cookie("dxl");
	var dxlmUrlValue2 = $.cookie("dxlm");
	if(dxlUrlValue2 =="2"){
		$("header").remove();
		$("footer").remove();
	};
	if(dxlmUrlValue2){
		$("#map ul li").eq(2).html("<a href='tel:'"+dxlmUrlValue2 + ">" + dxlmUrlValue2+"<i>联系商家</i></a>");

		$("#tooBar .contact a").attr({
			href: "tel:"+dxlmUrlValue2
		});
	};
	// -----------------------------------

	var huiDialog = $("#huiDialog");  //优惠 弹层
	var huiLi = $("#hui li");  //点击显示优惠弹层的入口元素
	var infoDialog = $("#infoDialog"); //酒店简介弹层v
	var yht = $("#yanhuiting"); //宴会厅
	var yhtLi = yht.find("li");  //宴会厅 li
	var yhtLiHeight = yhtLi.outerHeight();  //宴会厅 li 高
	var collect = $(".collect");  //收藏按钮
	var yuyueOrder = $("#yuyueOrder");  //预约看店
	var yhtDialog = $('#yhtDialog'); //宴会厅图片弹层
	var winW = $(window).width();
	var winH = $(window).height();
	var onLineDialog = $("#onLineDialog");  //在线咨询弹层
	var collectAction = dxlHttp.my + "ucenterjsonp/?act=onFavorite&";
	var map_fir = $("#map ul li a span:first-child").outerWidth();  //地图版块 商家地址元素宽
	var orderData = {
		hotelName : $("#topImg .topInfos h1").text(),  //酒店名 
		city_short : $.cookie("city")  //城市
	}
	var dqBtn = $(".dqBtn");  //档期查询下单  弹层按钮
	var dqDialog = $("#dqDialog");  //档期查询弹层
	
	//返回顶部
	$.mTopCall();
	
	//领取礼包 
	//!$.cookie("mobile") && $.dxlInclud(["hotelGiftDialog"]);

	//第三方合作隐藏头部
	$.cookie("headerHideForDxl2")==2 ? $("header").hide() : $("header").show();

	//返回
	$("header .prev").touchClick(function(ev){  
		ev.preventDefault();
		var url = $.cookie("hotelListUrl") ? $.cookie("hotelListUrl") : "/" + $.cookie("city") + "/HunYan/";
		window.location.href = url;	
	});
	
	//存下单url cookie
	$.cookie("orderUrl",window.location.href,{expires:7,domain:"daoxila." + s4Com,path:"/" });
	huiDialog.find(".dialogWrap").css("height",winH);
	
	//显示优惠弹层
	huiLi.touchClick(function(){	
		$.dialogOpen(huiDialog);
		$.dxlGaPageTracker("/VPTracker/WapHunYan/Order/Submobile-common?userType=" + dxlGALoginStatus + "&orderType=gift");  //虚拟url
	...
	//关闭优惠弹层
	huiDialog.find(".diaH3Tit span").touchClick(function(event){ 
		event.stopPropagation();
		$.dialogClose(huiDialog);
	});
	
	//优惠弹层下单
	if( $.cookie("user[mobile]") ){  //将cookie的手机号填入input
		huiDialog.find("input").val( $.cookie("user[mobile]") );
	}
	huiDialog.find(".toOrder").touchClick(function(){
		if( !huiDialog.find("input").val().isPhone() ){
			$.mAlert("您输入的手机号码有误，请核实后重新输入。");
			return false;	
		}
		$.dxlInclud(["M-hotelOrder"],function(){
			$.hotelOrder({
				mobile:huiDialog.find("input").val(),
				order_from:"Wap_" + orderData.city_short + "_HotelDetail_Gift_BottomRight",
				hotel:orderData.hotelName  //酒店 
			})
		}); 
		huiDialog.find("input").blur();
	})
	
	//显示酒店简介弹层
	$(".checkInfoDialog").touchClick(function(){
		$.dialogOpen(infoDialog);
		$.dxlGaPageTracker("/VPTracker/WapHunYan/(MeiLunDaJiuDian)-Intro"); 
		setInfoHeight();
	});
	//关闭酒店简介弹层
	infoDialog.find("span").touchClick(function(){
		$.dialogClose(infoDialog);
	});
	//简介内容高度伸缩
	function setInfoHeight(){  
		var infoHeight = $(".jianjieCon div").height();  //实际酒店简介的高度
		var jianjie = $(".jianjie");
		if( infoHeight > 174 ){
			if(!jianjie.find("span").size()){
				jianjie.append('<p><span></span></p>');
				jianjie.find("p").touchClick(function(){
					if(!$(this).hasClass("up")){
						jianjie.find(".jianjieCon").animate({"height":infoHeight},500);
						$(this).addClass("up");
					}else{
						jianjie.find(".jianjieCon").animate({"height":175},500);
						$(this).removeClass("up");
					}
	...
	//查看全部宴会厅
	if(yhtLi.length > 2){
		yht.find(".more").css("display","block");
		yht.find("ul").css( "height",2 * yhtLiHeight );
		yhtLi.eq(2).css("border-top",0);
		yht.find(".more").touchClick(function(){
			if(!$(this).hasClass("upMore")){
				yht.find("ul").animate({"height":yhtLi.length * yhtLiHeight} );
				$(this).addClass("upMore");
				$(this).find("span").html("收起");
				yhtLi.eq(2).css("border-top","1px solid #d7d7d7");
			}else{
				yht.find("ul").animate({"height":2 * yhtLiHeight });
				$(this).removeClass("upMore");
				$(this).find("span").html("查看全部" + yhtLi.length + "个宴会厅");
				yhtLi.eq(2).css("border-top",0);
			}
		})
	}
	
	//无评论时直接跳转添加评论页面，并且提示登录
	$("#pingLun .toCommentNow").touchClick(function(event){
		event.preventDefault();
		$.loginAndComment();
	});
	
	$(window).on("resize",function(){
		$.dialogAnimate(huiDialog,null,null,null);
		$.dialogAnimate(infoDialog,null,null,null);
		$.dialogAnimate(dqDialog,null,null,null);
		
		if(($("#map ul li:first-child a").width() - map_fir) > 210){
			$("#map .shopBusy").css("display","inline");
		}else{
			$("#map .shopBusy").css("display","block");
		}
	}).resize();
	
	//收藏
	collect.touchClick(function(){ 
		var dataUserId = $.cookie("user[line]");   // 判断用户是否登录
		var collectData = {
			type : 1,
			type_id : collect.data("id"),   //商户ID
			name : $("#topImg .topInfos h1").text()   //酒店名
		}
		$.dxlGaPageTracker("/VPTracker/WapHunYan/ShouCang");  //虚拟url
		if( dataUserId){
			if( collect.data("collect") == 0 ){
				collectSuccess();
			}else{
	...
	function collectGet(par){  
		par.IsSave = collect.data("collect");  
		$.getJSON(collectAction + $.param(par) + "&callback=?",function(data){})
	}
	
	//显示预约看店 弹层
	$(".yuyue").touchClick(function(){
		yuyueOrder.removeClass("showOutUp").addClass("showInDown").show();
		$.dxlBackgroundShow();
		yuyueOrder.find(".closeY").touchClick(yuyueOrderClose);
	});
	
	//预约看店 下单
	if( $.cookie("user[mobile]") ){  //将cookie的手机号填入input
		yuyueOrder.find(".mobile").val( $.cookie("user[mobile]") );
	}
	yuyueOrder.find(".toOrder").touchClick(function(){
		if(!yuyueOrder.find(".mobile").val().isPhone()){
			$.mAlert("您输入的手机号码有误，请核实后重新输入。");
			return false;	
		}
		$.dxlInclud(["M-hotelOrder"],function(){
			$.hotelOrder({
				mobile:yuyueOrder.find(".mobile").val(), 
				order_from:"Wap_" + orderData.city_short + "_HotelDetail_Reserve_Bottom",
				hotel:orderData.hotelName  //酒店 
			})
		}); 
		yuyueOrder.find(".mobile").blur();
		yuyueOrderClose();
		
	})
	//关闭
	function yuyueOrderClose(){
		yuyueOrder.addClass("showOutUp").delay(800).removeClass("showInDown");
		setTimeout(function(){
			yuyueOrder.hide();
			$.dxlBackgroundHide();
		},500);
	...
	//显示宴会厅弹层
	yht.find(".le").touchClick(function(){
		setYHTLeft(- (yht.find(".le")).index(this) * winW);
	})

	$(".topVrBtn").touchClick(function(){
		setYHTLeft(0);
	})

	$.dxlInclud(["dxlTouchEvent"], function(){
		$("#yhtDomList").dxlTouchEvent({
			tag: 'ul',
			tagList: ".yhtDom",
			distance: winW,
		});
	...
	yhtDialog.find(".top").touchClick(function(e){
		event.stopPropagation();
		$.dialogClose(yhtDialog);
	});

	//点击宴会厅弹层的vr  跳转
	$(".vrJumpBtn").touchClick(function(){
		window.open( $(this).attr("vrHref") );
	})

	function setYHTLeft(l){
		$.dialogOpen(yhtDialog);
		$('#yhtDomList ul').css({
			width: winW * $('.yhtDom').length,
			left: l,
		});
		$("#yhtDomList li").css("width", winW);
		$("#yhtDomList").css("margin-top", (winH - $("#yhtDomList ul").height())/2);
		$.dxlGaPageTracker("/VPTracker/WapHunYan/Order/SubMobile-common?orderType=YanHuiTing");  //虚拟url
	}
	
	//宴会厅弹层下单
	if( $.cookie("user[mobile]") ){  //将cookie的手机号填入input
		yhtDialog.find(".phoneNum").val($.cookie("user[mobile]"));
	}
	yhtDialog.find(".sure").touchClick(function(){
		if (!yhtDialog.find(".phoneNum").val().isPhone()) {
			$.mAlert("您输入的手机号码有误，请核实后重新输入。");
			return false;	
		}
		$.dxlInclud(["M-hotelOrder"],function(){
			$.hotelOrder({
				mobile:yhtDialog.find(".phoneNum").val(),
				order_from:"Wap_" + orderData.city_short + "_HYDetail_YanHuiTing_KanChangDi",
				hotel:orderData.hotelName  //酒店 
			})
		}); 
		yhtDialog.find(".phoneNum").blur();
	});
	
	//在线咨询
	$("#tooBar .online").touchClick(function(){
		onLineDialog.removeClass("showOutUp").addClass("showInDown").show().dxlLayerFixedShow("bottom");
		$.dxlBackgroundShow();
		//咨询
		onLineDialog.find(".toLine").touchClick($.dxlCustomer);
		//关闭
		onLineDialog.find(".noLine").touchClick(onLineDialogClose);
	})
	function onLineDialogClose(){
		onLineDialog.addClass("showOutUp").delay(800).removeClass("showInDown").dxlLayerFixedHide("bottom");  
		setTimeout(function(){
			onLineDialog.hide();
			$.dxlBackgroundHide();
		},500);
	...
	//档期查询弹层
	dqBtn.touchClick(function(){
		$.dialogOpen(dqDialog);
	})
	dqDialog.find(".diaH3Tit span").touchClick(function(){
		$.dialogClose(dqDialog);
	});
	...
	dqDialog.find(".dqCaleDate").text(year + "-" + mon + "-" + date);
	//日期选择
	$.dxlInclud(["dxlWapDatePicker"], function() {
		dqDialog.find(".dqCaleDate").off().touchClick(function() {
			$.dxlBackgroundShow();
			var lastDate = $(this).text();
			$.dxlWapDatePicker({
				defaultDate : lastDate,
				"myAction": function(dateSelected) {
					dqDialog.find(".dqCaleDate").text(dateSelected);
				}
			});
		});
	});
	//档期查询 下单
	if( $.cookie("user[mobile]") ){  //将cookie的手机号填入input
		dqDialog.find(".mobileIn").val( $.cookie("user[mobile]") );
	}
	dqDialog.find(".submit").touchClick(function(){
		if(!dqDialog.find(".mobileIn").val().isPhone()){
			$.mAlert("您输入的手机号码有误，请核实后重新输入。");
			return false;	
		}
		$.dxlInclud(["M-hotelOrder"],function(){
			$.hotelOrder({
				mobile:dqDialog.find(".mobileIn").val(), 
				order_from:"Wap_" + orderData.city_short + "_HYDetail_Button_Schedule",
				hotel:orderData.hotelName  //酒店 
			})
		}); 
		dqDialog.find(".mobileIn").blur();
	})
