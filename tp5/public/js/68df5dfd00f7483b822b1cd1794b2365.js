...
var dxlfs = [
	...
	{name:"hotelOrder",js:"order/js/hotelOrder/1602.js",css:"order/css/hotelOrder/1501.css"},
...
$(function() {
    ...
    // touch转click
    $('body').touchClick('.touchToClick', function(event) {
        event.preventDefault();
        $(event.target).click();
    });
    ...
    // 通用变量
    var $taocan = $('#taocan');
    var $dancheOrder = $('.bookPop.dcbook');
    var $taocanOrder = $('.bookPop.tcbook');
    var $album = $('#fengcai');
    var $info = $('#info');
    var $backToList = $('.secTitMain>.prev');

    // 顶部图片轮播
    $.dxlInclud(["touchSlider"], function() {
        $.touchSlider({
            slideCell: "#carImgShow",
            titCell: "#carImgShow .hd ul", //开启自动分页 autoPage:true ，此时设置 titCell 为导航元素包裹层
            mainCell: "#carImgShow .bd ul",
            effect: "left",
            autoPlay: true, //自动播放
            autoPage: true, //自动分页
            switchLoad: "_src" //切换加载，真实图片路径为"_src","src"可以设置加载时的背景图 
        });
    });

    // 费用说明收缩
    $info.find('.desc').height() > 33 && $info.find('.desc').addClass('lockH') && $info.find('.more').show();
    $info.find('.more').addClass('touchToClick').on('click', function() {
        if ($(this).hasClass('upMore')) {
            $(this).removeClass('upMore');
            $info.find('.desc').removeClass('freeH').addClass('lockH');
        } else {
            $(this).addClass('upMore');
            $info.find('.desc').addClass('freeH').removeClass('lockH');
        }
    });
    
    // 显示更多套餐
    $taocan.find('.tcNav ul').height() > 50 && $taocan.find('.tcNav ul').addClass('lock') && $taocan.find('.more').show();
    $taocan.find('.more').addClass('touchToClick').on("click", function() {
        if ($(this).hasClass('upMore')) {
            $(this).removeClass('upMore');
            $taocan.find('.tcNav ul').removeClass('free').addClass('lock');
        } else {
            $(this).addClass('upMore');
            $taocan.find('.tcNav ul').addClass('free').removeClass('lock');
        }
    });
    !$taocan.find('.tcNav ul li').length && $('#taocan').hide();

    // 套餐选择
    $taocan.find('.tcNav ul li').addClass('touchToClick').on('click', function() {
        var index = $(this).index();
        var followCar = $.parseJSON($(this).attr('data-gcar'));
        var followCarHtml = '', $listUl;
        var followCarDom = $taocanOrder.find('.gCarList');
        $(this).addClass('cur').siblings().removeClass('cur');
        $taocan.find('.tcCon .tcConItem').eq(index).show().siblings().hide();
        $.each(followCar, function(index, item) {
            followCarHtml += '<div class="gCar">';
            followCarHtml += '   <div class="carInfo clearfix">';
            followCarHtml += '		<div class="carlogo"><img src="' + item.img + '-wapliebiao"></div>';
            followCarHtml += '		<div class="cardes">'
            followCarHtml += '		   <span class="carname"><i class="t">跟车</i>' + item.brand + '</span>';
            followCarHtml += '		   <span class="carprice" originalprice="' + item.originalPrice + '">￥' + item.price + '/半天</span>';
            followCarHtml += '		</div>';
            followCarHtml += '	 </div>';
            followCarHtml += '	 <div class="carCount clearfix">';
            followCarHtml += '		<h3>数量</h3><div class="car-count">';
            followCarHtml += '		    <i class="down"></i><i class="num" numMin="' + item.count + '">' + item.count + '</i><i class="up"></i></div>';
            followCarHtml += '	 </div>';
            followCarHtml += '</div>';
        });
        followCarDom.empty().html(followCarHtml);
        $listUl = $('.tcSliderList ul').eq(index);
        $listUl.find('li').width($(window).width() * 0.4).css('margin-right', $(window).width() * 0.03);;
        $listUl.width($listUl.find('li').outerWidth(true) * $listUl.find('li').length + 3);
        setPrice();
    });
    $taocan.find('.tcNav ul li').eq(0).click();

    // 套餐滑动效果
    $taocan.find('.tcConItem').each(function(index, dom) {
        var $lists = $(dom).find('.tcSliderList ul li');
        $(dom).find('.tcSliderList').css('position', 'relative');
        var $ulDom = $(dom).find('.tcSliderList ul').css({
            'position': 'relative',
            'width': $lists.outerWidth(true) * $lists.length + 1
        });
        $(dom).data('positionLeft', 0).on({
            'touchstart': function(event) {
                $(this).data('startX', event.originalEvent.touches[0].clientX);
                $(this).data('startY', event.originalEvent.touches[0].clientY);
            },
            'touchmove': function(event) {
                var dx = event.originalEvent.touches[0].clientX - $(this).data('startX');
                var dy = event.originalEvent.touches[0].clientY - $(this).data('startY');
                Math.abs(dy) < Math.abs(dx) && event.preventDefault();
                var newLeft = event.originalEvent.touches[0].clientX - $(this).data('startX') + $(dom).data('positionLeft');
                $(this).data('left', newLeft);
                $ulDom.css('left', newLeft);
            },
            'touchend': function(event) {
                var nowLeft = $(this).data('left');
                if (nowLeft < $ulDom.parent().width() - $ulDom.width())
                    nowLeft = $ulDom.parent().width() - $ulDom.width();
                if (nowLeft > 0)
                    nowLeft = 0;
                $(this).data('positionLeft', nowLeft);
                $ulDom.animate({
                    'left': nowLeft
                }, 200);
            }
        ...
        $(window).on('resize', function() {
            // UC不识css：vw的表示方法
            $lists.width($(window).width() * 0.4).css('margin-right', $(window).width() * 0.03);
            $(dom).find('.tcSliderList ul').css('width', $lists.outerWidth(true) * $lists.length + 1);
        });
    });

    // 单车下单
    $('#choice, #tool .order').addClass('touchToClick').on("click", function(event) {
        $.dxlBackgroundShow();
        $dancheOrder.slideDown();
    });
    $dancheOrder.find('.close').addClass('touchToClick').on('click', function() {
        $dancheOrder.slideUp(function() {
            $.dxlBackgroundHide();
        });
    });

    // 套餐下单
    $('.tcpiRi').addClass('touchToClick').on("click", function() {
        $.dxlBackgroundShow();
        $taocanOrder.slideDown();
    });
    $taocanOrder.find('.close').addClass('touchToClick').on('click', function() {
        $taocanOrder.slideUp(function() {
            $.dxlBackgroundHide();
        });
    });

    // 套餐数量选择
    $('.bookPop').addClass('touchToClick').on('click', '.car-count>.down, .car-count>.up', function() {
        var numDom = $(this).closest('.car-count').find('.num');
        var limit = parseInt(numDom.attr('nummin'), 10) || 1;
        var nowNum = parseInt(numDom.html(), 10);
        var plus = $(this).hasClass('up');
        nowNum = nowNum + (plus ? 1 : -1);
        if (nowNum < limit) return;
        numDom.html(nowNum);
        setPrice('数量选择');
    });

    // 头车颜色选择
    var colorSetter = $('.bookPop').find('.bookblock.xh ul.carVis li');
    var colorPicker = $('.bookPop').find('.bookblock.ys ul');
    colorSetter.addClass('touchToClick').on('click', function() {
        var colors = $.parseJSON($(this).attr('data-colors'));
        var colorContainer = $(this).closest('.bookPop').find('.bookblock.ys ul');
        var colorHtml = '';
        $.each(colors, function(index, item) {
            colorHtml += '<li id="' + item.id + '" price="' + item.price + '" originPrice="' + item.originalPrice + '" style="background: ' + item.color + '"></li>';
        });
        $(this).addClass('cur').siblings().removeClass('cur');
        colorContainer.html(colorHtml);
        colorContainer.find('li').eq(0).click();
    });
    colorPicker.addClass('touchToClick').on('click', 'li', function() {
        var thisPrice = $(this).attr('price');
        $(this).addClass('cur').siblings().removeClass('cur');
        $(this).closest('.bookPop').find('.carprice').eq(0).attr('originPrice', $(this).attr('originprice')).html('￥' + thisPrice + '/半天');
        $(this).closest('.bookPop').hasClass('tcbook') && setPrice('颜色选择');
    });

    // 默认选择型号
    $dancheOrder.find('ul.carVis li').eq(0).click();
    $taocanOrder.find('ul.carVis li').eq(0).click();

    // 下单
    $.dxlInclud(['M-huncheOrder', 'dxlMLogin'], function() {
        var $orders = $('.bookPop.dcbook, .bookPop.tcbook');
        $orders.find('.booknow').addClass('touchToClick').on('click', function() {
            var ifTaocan = $taocanOrder.filter(':visible').length ? true : false;
            $(this).closest('.bookPop').find('.close').click();
            $.mLogin({
                loginOk: orderForHunche,
                registerOk: orderForHunche
            });
            function orderForHunche() {
                var num = 1;
                var orderFrom = $.cookie('city') + '_' + (ifTaocan ? 'uWap_HunCheOrder_Confirm_bottomTop' : 'Wap_HunCheDetail_Booking_RightTop');
                ifTaocan ? $('.gCar .num').each(function(index, dom) {
                    num += parseInt($(dom).text(), 10);
                }) : num = $dancheOrder.find('.num').text();
                var params = {
                    order_from: orderFrom,
                    isTaocan: ifTaocan,
                    Total: num
                };
                ifTaocan ? params.Cost = $('.tcprice:visible').text() : params.carName = $dancheOrder.find('.cardes .carname').text();
                $.mHuncheOrder(params, {
                    orderOk: function() {
                        $.mAlert('预订成功');
                    },
    ...
    // 手机相册浏览
    $.dxlInclud(['touchAlbums'], function() {
        var picObjs = $.parseJSON($('#fengcai').attr('data-album'));
        var $closeAlbum = $('.closeAlbum'), $albumSilbings;
        var pics = [], option = {
            container: "#AlbumShow",
            containerBackground: '#2d2821',
            loadingBackground: 'url(' + dxlHttp.s4 + 'miyue/img/index-p/loading.gif) no-repeat center',
            changeZoomOut: false,
            curIndex: 1,
            prepareNum: 2,
            badImg: dxlHttp.s4 + "www/img/index/wish.png"
        };
        
        $.each(picObjs, function(index, obj) {
            pics.push(obj.originalImage);
        });
        $album.find("ul li").addClass('touchToClick').on("click", function() {
            $album.data('nowheight', $(window).scrollTop());
            $(window).scrollTop(0);
            showAlbumPics($(this).index() || 1);
        });
        
        $closeAlbum.addClass('touchToClick').on("click", function() {
            $albumSilbings.show();
            $(option.container).remove();
            $closeAlbum.hide();
            $(window).scrollTop($album.data('nowheight'));
        });
        
        function showAlbumPics (index) {
            option.curIndex = index;
            $.dxlAlbumsTouchSlides(pics, option);
            $albumSilbings = $(option.container).siblings(':visible').hide();
            $closeAlbum.show();
        }
    });
    
    // 防止点透
    $('body').addClass('touchToClick').on("click", '#dxlMOrderLayer .closeDialog', function(event) {
        $backToList.data('overLayerClick', event.timeStamp);
    });
    $backToList.addClass('touchToClick').on('click', function(event) {
        var overLayerClickTime = $backToList.data('overLayerClick') || 0;
        var thisClickTime = event.timeStamp || +new Date;
        event.preventDefault();
        thisClickTime - overLayerClickTime > 500 && (window.location.href = $(this).attr('href'));
    });
    
    // 在线咨询
    $('#tool>.custom').addClass('touchToClick').on("click", $.dxlCustomer);
    
    // UC DEBUG
    $(window).on('resize', function() {
        $('.autoH').css("max-height", $(window).height() * 0.56);
    }).resize();

    // 价格计算
    function setPrice(from) {
        setTimeout(countPrice, 0);

        function countPrice() {
            var originPrice, savePrice, nowPrice;
            var toucheDom = $taocanOrder.find('.carprice').eq(0);
            var touchePrice = parseInt($.trim(toucheDom.html()).substr(1), 10);
            var toucheOriginPrice = parseInt($.trim(toucheDom.attr('originprice')), 10);
            var followCars = $taocanOrder.find('.gCarList'),
                followCarsPrice = 0,
                followCarsOriginPrice = 0;
            var priceDomToSet = $('#taocan').find('.tcCon .tcConItem:visible')
            followCars.find('.gCar').each(function(index, item) {
                followCarsPrice += parseInt($(item).find('.num').html()) * parseInt($(item).find('.carprice').html().substr(1), 10);
                followCarsOriginPrice += parseInt($(item).find('.num').html()) * parseInt($(item).find('.carprice').attr('originalprice'), 10);
            });
            originPrice = toucheOriginPrice + followCarsOriginPrice;
            nowPrice = touchePrice + followCarsPrice;
            nowPrice *= parseInt($taocan.find('.tcNav ul li.cur').attr('data-discount'), 10) / 100;
            nowPrice = Math.round(nowPrice / 10) * 10;
            originPrice = Math.round(originPrice / 10) * 10;
            savePrice = originPrice - nowPrice;
            priceDomToSet.find('.tcprice').html(nowPrice);
            priceDomToSet.find('.origprice').html(originPrice);
            priceDomToSet.find('.sprice').html(savePrice);
        }
    }
